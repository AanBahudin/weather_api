...
const HourlyForcast = () => {
    ...
    return (
        <section className="mt-5 mb-20">
            <div data-aos="fade-up" className="flex bg-white/[25%] backdrop-blur-3xl dark:bg-cardDark rounded-md p-5 flex-row justify-around duration-200 self-stretch w-[96%] gap-x-4 mx-auto ">
                    ...
                    filtering.slice(0,5).map((item, index) => {
                        ...
                        return (
                            <section key={index} className="text-sm text-center text-white py-2 px-4 w-[20%] h-fit rounded-lg">
                                <h1 className="text-[1.7rem] font-kanit text-center text-sky-800 dark:text-silver mb-2">{item.time.split(' ')[1]}</h1>
                                <img src={icon} className="mx-auto w-32" alt="icon" />
                                <h1 className="text-[2rem] font-dosis">{item.temp_c} °C</h1>
                                <h1 className="text-center text-sky-800 dark:text-darkSecondary text-[1.2rem] font-kanit mt-4">{text}</h1>
